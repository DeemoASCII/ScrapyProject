...
class JobRedisMiddlewares(object):
    '''
    把除了首页的每一个url都放入redis数据库中防止重复爬取，第一页可以重复抓
    '''

    def __init__(self):
        self.db = redis.StrictRedis(host='localhost', port=6379, db=2)

    def process_request(self, request, spider):
        if request.url.startswith('https://jobs.51job.com/') == True:
            url_md5 = hashlib.md5(request.url.encode()).hexdigest()
            result = self.db.sadd('job51_url', url_md5)
            if result == False:
                raise IgnoreRequest
